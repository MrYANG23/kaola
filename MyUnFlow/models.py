...
class FlowNet2S(FlowNetS.FlowNetS):
    ...
    def forward(self, inputs):
        ...
        print(inputs.contiguous().view(inputs.size()[:2]+(-1,)).size())
        rgb_mean = inputs.contiguous().view(inputs.size()[:2]+(-1,)).mean(dim=-1).view(inputs.size()[:2] + (1,1,1,))
        print('rgb_mean:',rgb_mean.size())
        x = (inputs - rgb_mean) / self.rgb_max
...
if __name__ == '__main__':
    a=torch.randn(1,3,2,256,256)
    flownet2s=FlowNet2S()
    flownet2c=FlowNet2C()
    flownet2cs=FlowNet2CS()
    flownet2=FlowNet2()
    # a1,a2,a3,a4,a5=flownet2s(a)
    # b1=flownet2cs(a)
