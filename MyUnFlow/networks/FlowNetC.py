...
class FlowNetC(nn.Module):
    def __init__(self, batchNorm=True, div_flow = 20,training=False):
        ...
        for m in self.modules():
            ...
            if isinstance(m, nn.ConvTranspose2d):
                if m.bias is not None:
                    init.uniform_(m.bias)
                init.xavier_uniform_(m.weight)
                # init_deconv_bilinear(m.wbatchNormeight)
...
if __name__ == '__main__':
    ...
    a=torch.randn(4,6,1152,320)
    flownetc=FlowNetC(training=True)
    a1=flownetc(a)
    print(type(a1))
    print(len(a1))
    print(a1[0].shape)
